...
const validateEmail = (email) => {
  const emailReg = /^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+@[a-zA-Z0-9]+\.(net|org|com)\b/;
...
const validatePhoneNumber = (phoneNumber) => {
  const five = /^(\(\d{3}\)|\d{3}) ?-?\d{3} ?-?\d{4}$/;
  return five.test(phoneNumber);
  // Solution code here...
};
...
const findTagNames = (elements) => {
  return elements;
  //this isn't me tryna to solve, the red underline was bothering the heck outta me
  // Solution code here...
};
